...
void Example_StaticUpdate(void) { Example->exampleStaticVar++; }
...
void Example_StageLoad(void) { RSDK.PrintLog(SEVERITY_NONE, "Example StageLoad()\n"); }
// Run once per frame, once per entity, used to render sprites in editor (never used in-game)
void Example_EditorDraw(void) { RSDK.PrintLog(SEVERITY_NONE, "Example EditorDraw()\n"); }
// Run once per object, used to load assets in the editor (never used in-game)
void Example_EditorLoad(void) { RSDK.PrintLog(SEVERITY_NONE, "Example EditorLoad()\n"); }
